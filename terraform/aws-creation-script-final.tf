...
# Runs automatically on each student's dedicated AWS account
...
# ── Route53 Private Hosted Zone + DNS Records ────────────────────────

resource "aws_route53_zone" "private_zone" {
  name    = "infolab.com"
  comment = "Private hosted zone for Infoblox Lab"

  dynamic "vpc" {
    for_each = aws_vpc.eu_vpcs
    content {
      vpc_id     = vpc.value.id
      vpc_region = data.aws_availability_zones.available.id
    }
  }
...
output "dns_records" {
  value = [for k, v in var.eu_west_vpcs : "${v.app_fqdn} => ${v.private_ip}"]
}
